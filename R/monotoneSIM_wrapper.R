...
#'
#' The unknown link function, \eqn{g(.)} is approximated by a B-Spline basis expansion with coefficients \eqn{\xi} and basis functions defined using \code{knots}: \eqn{-1 \le u_0 < u_1 < \dots < u_L = 1}. If \code{monotone = TRUE}, a truncated Normal \eqn{_+ (0, \Sigma_\xi)} is assigned to \eqn{\xi} and the posterior samples is generated by an exact Hamiltonian Monte Carlo Algorithm. If \code{monotone = FALSE}, Normal\eqn{(0, \Sigma_\xi)} prior is assigned and the samples are generated from the corresponding posterior Normal distribution.
...
#'
#' A normalized Gaussian prior with mean 0 and variance \eqn{\sigma_\beta^2}is assigned on the single index parameter, \eqn{\beta} and the posterior is generated using an Elliptical Slice Sampler.
#'
...
#'
#' @param xi.init \eqn{(L+1) x 1} Vector; Starting value of B-Spline Basis Coefficients, \eqn{\xi} for the algorithm.
#'
